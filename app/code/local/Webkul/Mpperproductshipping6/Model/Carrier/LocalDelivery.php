...
class Webkul_Mpperproductshipping6_Model_Carrier_LocalDelivery extends Mage_Shipping_Model_Carrier_Abstract
{
    /* Use group alias */
    protected $_code = 'mpperproductshipping6';

    public function collectRates(Mage_Shipping_Model_Rate_Request $request)
    {
        //skip if not enabled
        if (!Mage::getStoreConfig('carriers/' . $this->_code . '/active') || Mage::getStoreConfig('carriers/mp_multi_shipping6/active')) {
            return false;
        }
        ...
        foreach ($shippingdetail as $shipdetail) {
            ...
            foreach ($session->getQuote()->getAllItems() as $item) {
                if (in_array($item->getId(), $itemsarray)) {
                    $mpshippingcharge = Mage::getModel('catalog/product')->load($item->getProductId())->getMpShippingCharge6();
            ...
            $_SESSION['shipping_price_6'] = $price;
            //	Mage::log($_SESSION['shipping_price_6'], null, 'shipping_price_6.log', true );
