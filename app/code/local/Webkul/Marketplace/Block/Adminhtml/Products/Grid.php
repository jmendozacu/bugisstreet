        ...
        foreach ($this->getCollection() as $item) {
            ...
            if(!(is_null($item->getmageproductid())) && $item->getmageproductid() != 0){
                 if($item->getstatus() == 1){
                    ...
                    $item->status = sprintf('<a href="%s" title="Click to Approve" onclick="return confirm(\'You sure?\')">Unapproved</a>',$this->getUrl('adminhtml/marketplace_products/approve/id/' . $item->getmageproductid()));
                }
