...
//						$shipaccordingtoseller[$shipdata['seller_id']]['item_ids']=$shipdata['item_ids'];
                    ...
                    Mage::log($shipaccordingtoseller[$shipdata['item_ids']]['seller_id'], null, 'sellerdata.log', true);
                }
            }
        }
        ...
        foreach($shipaccordingtoseller as $seller_id=>$infoseller){
            ...
            foreach ($session->getQuote()->getAllVisibleItems() as $item) {
                ...
                //$productowner->getSelect()->group('index_id');
                foreach($productowner as $product){
                    ...
                    $i = 0;
                    foreach($list_shippings as $shipings){
                        //Mage::log($shipings, null, 'sellerdata.log', true);
                        $_title = Mage::getStoreConfig("carriers/$shipings/title");
                        $method.="<li><input type='radio' class='custom_ship' name='".$seller_id."' value='".$arrShipCharge[$seller_id]['cost']."'></input><label class='mpmultishiplabel'.$i>".$_title."</label></li>";
                        $i ++;
                    }
