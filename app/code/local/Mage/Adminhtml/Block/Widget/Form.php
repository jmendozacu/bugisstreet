    ...
    protected function _setFieldset($attributes, $fieldset, $exclude=array())
    {
        ...
        foreach ($attributes as $attribute) {
            ...
            if ( ($inputType = $attribute->getFrontend()->getInputType())
                ...
                if ($inputType == 'select') {
                    if($attribute["attribute_code"]=="group_id"){
                        $array_group = array();
                        foreach($attribute->getSource()->getAllOptions(true, true) as $row1){
                            $groups = Mage::getResourceModel('customer/group_collection')
                                ->addFieldToFilter('customer_group_id', $row1['value'])
                                ->load();
                            ...
                            if($limit==1){
                                $array = array();
                                $array["value"] = $row1['value'];
                                $array["label"] = $row1['label'];
                                $array_group[]= $array;

                            }
                        }
                        $element->setValues($array_group);
                    }
                    else{
