...
class Mage_Adminhtml_Block_Customer_Group_Edit_Form extends Mage_Adminhtml_Block_Widget_Form
{
    ...
    protected function _prepareLayout()
    {
        ...
        $member[0] = array();
        $member[0]['value']='1';
        $member[0]['label']="Customer";
        $member[1] = array();
        $member[1]['value']='2';
        $member[1]['label']="Seller";
       // var_dump($member);
        $fieldset->addField('member_type', 'select',
            array(
                'name'  => 'member_type',
                'label' => Mage::helper('customer')->__('Membership Type'),
                'title' => Mage::helper('customer')->__('Membership Type'),
                'class' => 'required-entry',
                'required' => true,
                'values' => $member
            )
        );
       // var_dump(Mage::getSingleton('tax/class_source_customer')->toOptionArray());
       // die();

        $fieldset->addField('product_listing', 'text',
            array(
                'name'  => 'product_listing',
                'label' => Mage::helper('customer')->__('Product Listing'),
                'title' => Mage::helper('customer')->__('Product Listing'),
                'class' => 'validate-number',
            )
        );


        $fieldset->addField('transaction_percent', 'text',
            array(
                'name'  => 'transaction_percent',
                'label' => Mage::helper('customer')->__('Transaction Percent'),
                'title' => Mage::helper('customer')->__('Transaction Percent'),
                'class' => 'validate-number',
            )
        );
