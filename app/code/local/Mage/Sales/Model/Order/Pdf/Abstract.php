...
abstract class Mage_Sales_Model_Order_Pdf_Abstract extends Varien_Object
{
    ...
    protected function insertOrder(&$page, $obj, $putOrderId = true)
    {
        ...
        // Tung add payment method
        //$paymentCode = $order->getPayment()->getMethod();
        //$payment_method = Mage::getStoreConfig('payment/'.$paymentCode.'/title');

        $payment_method = $order->getPayment()->getMethodInstance()->getTitle();
        $cc_number = $order->getPayment()->getCcLast4();
        $sType = $order->getPayment()->getCcType();
        $aType = Mage::getSingleton('payment/config')->getCcTypes();
        $page->drawText(strip_tags(trim($payment_method)), $paymentLeft, $yPayments, 'UTF-8');
        $yPayments -= 15;
        if (isset($aType[$sType])) {
            $cc_type = $aType[$sType];
            $page->drawText('Credit Card Type: ' . strip_tags(trim($cc_type)), $paymentLeft, $yPayments, 'UTF-8');
            $yPayments -= 15;
        }
        if(isset($cc_number)){
            $page->drawText('Credit Card Number: xxxx-' . strip_tags(trim($cc_number)), $paymentLeft, $yPayments, 'UTF-8');
            $yPayments -= 15;
        }
        ...
        foreach ($payments as $paymentCode=>$paymentModel) {
            $paymentTitle = Mage::getStoreConfig('payment/'.$payments.'/title');
            $methods[$paymentCode] = array(
                'label'   => $paymentTitle,
                'value' => $paymentCode,
            );
        }
        $page->drawText(strip_tags(trim($methods)), $paymentLeft, $yPayments, 'UTF-8');
        $yPayments -= 15;*/
        // End
        ...
        if ($order->getIsVirtual()) {
            ...
            foreach($seller as $row){
                if($row!=$id){
                    $str = preg_replace("#<p class=\"".$row."\">(.*?)</p>#is", "", $str);
                }
            }
            $str=preg_replace('/class=".*?"/', '', $str);

            foreach (Mage::helper('core/string')->str_split($str, 45, true, true) as $_value) {
            ...
            if (count($tracks)) {
                $page->setFillColor(new Zend_Pdf_Color_Rgb(0.93, 0.92, 0.92));
                $page->setLineWidth(0.5);
                $page->drawRectangle(285, $yShipments, 550, $yShipments - 10);
                $page->drawLine(490, $yShipments, 490, $yShipments - 10);
                ...
                $page->drawText(Mage::helper('sales')->__('Number'), 500, $yShipments - 7, 'UTF-8');
                ...
                foreach ($tracks as $track) {
                    ...
                    $page->drawText($track->getNumber(), 500, $yShipments , 'UTF-8');
