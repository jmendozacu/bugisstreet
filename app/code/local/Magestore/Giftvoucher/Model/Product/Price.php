...
class Magestore_Giftvoucher_Model_Product_Price extends Mage_Catalog_Model_Product_Type_Price
{
	...
	protected function _applyOptionsPrice($product, $qty, $finalPrice){
		if ($amount = $product->getCustomOption('amount')){
			...
			$voucherCurrency = Mage::getModel('directory/currency')->load($product->getCustomOption('currency_giftcode')->getValue());
			$baseCurrency = Mage::getModel('directory/currency')->load(Mage::app()->getStore()->getBaseCurrencyCode());
			
			$finalPrice = $finalPrice * $finalPrice / $baseCurrency->convert($finalPrice, $voucherCurrency);
		}
