    ...
    public function paypalPrepareItems($observer) {
        ...
        if (version_compare(Mage::getVersion(), '1.4.2', '>=')) {
            ...
            if ($paypalCart) {
				...
				if (!$giftCardDiscount) $giftCardDiscount = $session->getGiftVoucherDiscount();
                if ($giftCardDiscount)
				...
				if (!$giftCardCreditAmount) $giftCardCreditAmount = $session->getUseGiftCreditAmount();
    ...
    protected function _addGiftVoucherForOrder($order) {
        foreach ($order->getAllItems() as $item) {
            ...
            for ($i = 0; $i < $item->getQtyOrdered() - $giftVouchers->getSize(); $i++) {
				...
				$giftVoucher->setCurrency($buyRequest['currency_giftcode']);
