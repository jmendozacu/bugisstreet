 ...
 */
class Ocean_CustomerAttributes_Helper_Eav extends Mage_Core_Helper_Data
{
    /**
    ...
    public function getAttributeFormOptions()
    {
        return array(
            array(
                'label' => Mage::helper('customer_attributes')->__('Default EAV Form'),
                'value' => 'default'
            )
        );
    }
            ...
            case 'multiline':
                if (isset($validateRules['min_text_length']) && isset($validateRules['max_text_length'])) {
                    ...
                    if ($minTextLength > $maxTextLength) {
                        $errors[] = Mage::helper('customer_attributes')->__('Wrong values for minimum and maximum text length validation rules.');
                    }
                }
                break;
            case 'date':
                if (isset($validateRules['date_range_min']) && isset($validateRules['date_range_max'])) {
                    ...
                    if ($minValue > $maxValue) {
                        $errors[] = Mage::helper('customer_attributes')->__('Wrong values for minimum and maximum date validation rules.');
                    }
                }
                break;
            default:
    ...
    {
        return array(
            'alphanumeric'  => Mage::helper('customer_attributes')->__('Alphanumeric'),
            'numeric'       => Mage::helper('customer_attributes')->__('Numeric Only'),
            'alpha'         => Mage::helper('customer_attributes')->__('Alpha Only'),
            'url'           => Mage::helper('customer_attributes')->__('URL'),
            'email'         => Mage::helper('customer_attributes')->__('Email'),
            'date'          => Mage::helper('customer_attributes')->__('Date'),
        );
    }
    ...
    {
        return array(
            'striptags'     => Mage::helper('customer_attributes')->__('Strip HTML Tags'),
            'escapehtml'    => Mage::helper('customer_attributes')->__('Escape HTML Entities'),
            'date'          => Mage::helper('customer_attributes')->__('Normalize Date')
        );
    }
    ...
    public function getAttributeInputTypes($inputType = null)
    {
        $inputTypes = array(
            'text'          => array(
                'label'             => Mage::helper('customer_attributes')->__('Text Field'),
            ...
            'textarea'      => array(
                'label'             => Mage::helper('customer_attributes')->__('Text Area'),
                'manage_options'    => false,
                'validate_types'    => array(
                    'min_text_length',
                    'max_text_length',
            ...
            'multiline'     => array(
                'label'             => Mage::helper('customer_attributes')->__('Multiple Line'),
                'manage_options'    => false,
                'validate_types'    => array(
                    'min_text_length',
                    'max_text_length',
            ...
            'date'          => array(
                'label'             => Mage::helper('customer_attributes')->__('Date'),
                'manage_options'    => false,
                'validate_types'    => array(
                    'date_range_min',
                    'date_range_max'
            ...
            'select'        => array(
                'label'             => Mage::helper('customer_attributes')->__('Dropdown'),
                'manage_options'    => true,
            ...
            'multiselect'   => array(
                'label'             => Mage::helper('customer_attributes')->__('Multiple Select'),
                'manage_options'    => true,
            ...
            'boolean'       => array(
                'label'             => Mage::helper('customer_attributes')->__('Yes/No'),
                'manage_options'    => false,
                'validate_types'    => array(),
                'validate_filters'  => array(),
                'filter_types'      => array(),
            ...
            'file'          => array(
                'label'             => Mage::helper('customer_attributes')->__('File (attachment)'),
                'manage_options'    => false,
                'validate_types'    => array(
                    'max_file_size',
                    'file_extensions'
            ...
            'image'         => array(
                'label'             => Mage::helper('customer_attributes')->__('Image File'),
                'manage_options'    => false,
                'validate_types'    => array(
                    'max_file_size',
                    'max_image_width',
        ...
        if ($data) {
            ...
            if (isset($data['attribute_code'])) {
                ...
                if (!$validatorAttrCode->isValid($data['attribute_code'])) {
                    Mage::throwException(Mage::helper('customer_attributes')->__('Attribute code is invalid. Please use only letters (a-z), numbers (0-9) or underscore(_) in this field, first character should be a letter.'));
                }
            }
        }
        return $data;
