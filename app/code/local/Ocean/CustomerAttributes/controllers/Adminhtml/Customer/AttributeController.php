 ...
 */
class Ocean_CustomerAttributes_Adminhtml_Customer_AttributeController extends Mage_Adminhtml_Controller_Action
{
    /**
    ...
    protected function _initAction()
    {
        $this->loadLayout()
            ...
            ->_addBreadcrumb(
            Mage::helper('customer_attributes')->__('Customer'),
            Mage::helper('customer_attributes')->__('Customer'))
            ->_addBreadcrumb(
            Mage::helper('customer_attributes')->__('Manage Customer Attributes'),
            Mage::helper('customer_attributes')->__('Manage Customer Attributes'));
        return $this;
    }
    ...
    public function editAction()
    {
        ...
        if ($attributeId) {
            ...
            if (!$attributeObject->getId()) {
                $this->_getSession()
                    ->addError(Mage::helper('customer_attributes')->__('Attribute is no longer exists.'));
                $this->_redirect('*/*/');
                return;
            }
            if ($attributeObject->getEntityTypeId() != $this->_getEntityType()->getId()) {
                $this->_getSession()->addError(
                    Mage::helper('customer_attributes')->__('You cannot edit this attribute.'));
                $this->_redirect('*/*/');
                return;
            }
        ...
        $label = $attributeObject->getId()
            ? Mage::helper('customer_attributes')->__('Edit Customer Attribute')
            : Mage::helper('customer_attributes')->__('New Customer Attribute');
    ...
    public function validateAction()
    {
        ...
        if (!$attributeId) {
            ...
            if ($attributeObject->getId()) {
                $this->_getSession()->addError(
                    Mage::helper('customer_attributes')->__('Attribute with the same code already exists')
                );
    ...
    {
        return Mage::helper('customer_attributes/customer')->filterPostData($data);
    }
    ...
    public function saveAction()
    {
        ...
        if ($this->getRequest()->isPost() && $data) {
            /* @var $attributeObject Mage_Customer_Model_Attribute */
            $attributeObject = $this->_initAttribute();
            /* @var $helper Ocean_CustomerAttributes_Helper_Data */
            ...
            try {
                Mage::dispatchEvent('customer_attribute_before_save', array(
                    'attribute' => $attributeObject
                ));
                $attributeObject->save();
                Mage::dispatchEvent('customer_attribute_save', array(
                    'attribute' => $attributeObject
                ));

                $this->_getSession()->addSuccess(
                    Mage::helper('customer_attributes')->__('The customer attribute has been saved.')
                );
                ...
                $this->_getSession()->addException($e,
                    Mage::helper('customer_attributes')->__('An error occurred while saving the customer attribute.')
                );
                $this->_getSession()->setAttributeData($data);
                $this->_redirect('*/*/edit', array('_current' => true));
                return;
    ...
    public function deleteAction()
    {
        ...
        if ($attributeId) {
            ...
            {
                $this->_getSession()->addError(
                    Mage::helper('customer_attributes')->__('You cannot delete this attribute.')
                );
                $this->_redirect('*/*/');
                return;
            }
            try {
                ...
                Mage::dispatchEvent('customer_attribute_delete', array(
                    'attribute' => $attributeObject
                ));

                $this->_getSession()->addSuccess(
                    Mage::helper('customer_attributes')->__('The customer attribute has been deleted.')
                );
                $this->_redirect('*/*/');
                return;
            } catch (Mage_Core_Exception $e) {
                ...
                $this->_getSession()->addException($e,
                    Mage::helper('customer_attributes')->__('An error occurred while deleting the customer attribute.')
                );
                $this->_redirect('*/*/edit', array('attribute_id' => $attributeId, '_current' => true));
                return;
            }
